...
local icons = require("misc.status.icons")
local colors = require("misc.status.color")
...
Items.diff_summary = function()
	...
	if minidiff_summary then
		...
		if minidiff_summary.source_name == "git" then
			...
			table.insert(items, { hl = "DiffText", text = "git " })
		end
...
Items.diagnostics = function()
	...
	for severity, count in pairs(local_counts) do
		if count > 0 then
			-- local hl = "Diagnostic" .. severity:sub(1, 1) .. severity:sub(2):lower()
			local hl = "Normal"
			table.insert(
				parts,
				H.add_highlight2(hl, string.format("%s %d", icons.diagnostics[severity], local_counts[severity], count))
			)
		end
	end
