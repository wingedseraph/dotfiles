...
statuscolumn.line_numbers = {
	...
	init = function(self, segmantConfig)
		...
		if colors.current_line ~= nil then
			vim.api.nvim_set_hl(
				0,
				colors.current_line.hl_name,
				vim.tbl_extend("keep", colors.current_line.value, colors.extra_styles or {})
			)
...
statuscolumn.folds = {
	...
	validate = function(self)
		...
		for index, value in ipairs(statuscolumn.fold_positions[vim.api.nvim_get_current_buf()]) do
			if
				vim.fn.foldclosed(value.close) ~= value.close
				or vim.fn.foldclosedend(value.close_end) ~= value.close_end
				or vim.fn.foldlevel(vim.v.lnum) ~= value.level
			then
			...
			if vim.fn.foldclosed(vim.v.lnum) ~= -1 then
				...
				for index, value in ipairs(foldLocs[vim.api.nvim_get_current_buf()]) do
					...
					if value.close == vim.v.lnum then
						statuscolumn.fold_positions[vim.api.nvim_get_current_buf()][index] = vim.tbl_extend(
							"force",
							statuscolumn.fold_positions[vim.api.nvim_get_current_buf()][index],
							{
				...
				if vim.g.nestFolds == false then
					...
					color = self.folds[foldLevel].close_hl ~= nil and "%#" .. self.folds[foldLevel].close_hl .. "#"
						or ""
				else
					for f = 1, foldLevel - 1 do
						text = text
							.. (self.borders[f].hl ~= nil and "%#" .. self.borders[f].hl .. "#" or "")
							.. self.borders[f].normal
					end

					text = text
						.. (self.folds[foldLevel].close_hl ~= nil and "%#" .. self.folds[foldLevel].close_hl .. "#" or "")
						.. self.folds[foldLevel].close
				end
			else
				...
				for index, value in ipairs(foldLocs[vim.api.nvim_get_current_buf()]) do
					if value.close == vim.v.lnum then
						...
						if vim.g.nestFolds == true then
							text = self.folds[foldLevel].open_hl ~= nil
									and text .. "%#" .. self.folds[foldLevel].open_hl .. "#" .. self.folds[foldLevel].open
								or text .. self.folds[foldLevel].open
						else
							text = self.folds[foldLevel].open
							color = self.folds[foldLevel].open_hl ~= nil
									and "%#" .. self.folds[foldLevel].open_hl .. "#"
								or ""
						end
						...
						break
					elseif
						vim.fn.foldlevel(vim.v.lnum) == value.level
						and (vim.v.lnum > value.close and vim.v.lnum < value.close_end)
					then
						foldLevel = self:getFoldLevel(vim.fn.foldlevel(vim.v.lnum))

						if vim.g.nestFolds == true then
							text = self.borders[foldLevel].hl ~= nil
									and text .. "%#" .. self.borders[foldLevel].hl .. "#" .. self.borders[foldLevel].top
								or text .. self.borders[foldLevel].top
						else
							text = self.borders[foldLevel].top
							color = self.borders[foldLevel].hl ~= nil and "%#" .. self.borders[foldLevel].hl .. "#"
								or ""
						end
					...
					elseif vim.v.lnum > value.close and vim.v.lnum < value.close_end then
						...
						if vim.g.nestFolds == true then
							text = self.borders[foldLevel].hl ~= nil
									and text .. "%#" .. self.borders[foldLevel].hl .. "#" .. self.borders[foldLevel].normal
								or text .. self.borders[foldLevel].normal
						else
					...
					elseif value.close_end == vim.v.lnum then
						...
						if vim.g.nestFolds == true then
							text = self.borders[foldLevel].hl ~= nil
									and text .. "%#" .. self.borders[foldLevel].hl .. "#" .. self.borders[foldLevel].bottom
								or text .. self.borders[foldLevel].bottom
						else
							local foldNext = self:getFoldLevel(vim.fn.foldlevel(vim.v.lnum + 1))

							if foldNext ~= 0 then
								text = (
									self.borders[foldLevel].hl ~= nil
										and "%#" .. self.borders[foldLevel].hl .. "#" .. self.borders[foldLevel].mix_branch
									or self.borders[foldLevel].mix_branch
								)
									.. (
										self.borders[foldNext].hl ~= nil
											and "%#" .. self.borders[foldNext].hl .. "#" .. self.borders[foldNext].mix_tail
										or self.borders[foldNext].mix_tail
									)
							else
	...
	vim.api.nvim_create_autocmd({ "BufEnter", "BufAdd", "ModeChanged" }, {
		...
		callback = function()
			...
			rootParser:for_each_tree(function(TStree, languageTree)
				...
				local folds = vim.treesitter.query.parse(
					language,
					[[
					((_) @element
						(#any-match? @element
				...
				for captureID, captureNode, metadata, match in
					folds:iter_captures(TStree:root(), vim.api.nvim_get_current_buf())
				do
					table.insert(statuscolumn.buffers_with_folds, vim.api.nvim_get_current_buf())
					break
				end
